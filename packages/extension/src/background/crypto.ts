...
export const encryptForUi = async (value: string, encryptedSecret: string, privateKey: KeyLike) => {
  ...
  return await new EncryptJWT({ value }).setProtectedHeader({ alg: 'dir', enc: 'A256GCM' }).encrypt(symmetricSecret)
}
