...
export function parseConfig(config: Config) {
  ...
  if (!isObjectLike(storage) || !isFunction(storage.getItem) || !isFunction(storage.setItem)) {
  ...
  const maxTimeToLive = Math.min(config.maxTimeToLive ?? 0, Number.MAX_SAFE_INTEGER) || Infinity
  const serialize = isFunction(config.serialize) ? config.serialize : identity
  const deserialize = isFunction(config.deserialize) ? config.deserialize : identity
