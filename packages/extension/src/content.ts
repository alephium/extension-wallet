...
window.addEventListener('message', function (event: MessageEvent<WindowMessageType>) {
  // forward messages which were not forwarded before and belong to the extension
  if (!event.data?.forwarded && event.data?.extensionId === alephiumExtensionId) {
    sendMessage({ ...event.data })
  }
})
messageStream.subscribe(([msg]) => {
  window.postMessage({ ...msg, forwarded: true, extensionId: alephiumExtensionId }, window.location.origin)
})
