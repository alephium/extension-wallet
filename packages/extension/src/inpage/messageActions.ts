...
const extensionId = document.getElementById('alephium-extension')?.getAttribute('data-extension-id')
...
export function waitForMessages<K extends MessageType['type'], T extends { type: K } & MessageType>(
  types: K[],
  timeout: number,
  predicate: (x: T) => boolean = () => true
): Promise<T extends { data: infer S } ? S : undefined> {
  return new Promise((resolve, reject) => {
    ...
    const handler = (event: MessageEvent<WindowMessageType>) => {
      if (types.find((e) => e === event.data.type) && predicate(event.data as any)) {
        ...
        return resolve(('data' in event.data ? event.data.data : undefined) as any)
      }
    }
