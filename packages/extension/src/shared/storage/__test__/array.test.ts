...
describe('ArrayStorage()', () => {
  describe('with primitives', () => {
    ...
    beforeAll(() => {
      arrayStorage = new ArrayStorage([1, 2, 3], 'arrayNumberTest', chromeStorageMock)
    })
    ...
    test('should add with selector function', async () => {
      ...
      expect(await arrayStorage.get()).toEqual([1, 2, 3, 4, 5, 6, 10, 20, 30, 40, 50, 60])
    })
    test('should remove single value', async () => {
      await arrayStorage.remove(4)
      expect(await arrayStorage.get()).toEqual([1, 2, 3, 5, 6, 10, 20, 30, 40, 50, 60])
    })
    test('should remove multiple values', async () => {
      await arrayStorage.remove([5, 6])
      expect(await arrayStorage.get()).toEqual([1, 2, 3, 10, 20, 30, 40, 50, 60])
    })
  ...
  describe('object with subscriptions and custom compare function', () => {
    ...
    test('should get with selector', async () => {
      expect(await arrayStorage.get((x) => x.id % 2 === 0)).toEqual([{ id: 2, value: 'b' }])
      expect(onChangeHandler).toHaveBeenCalledTimes(0)
    })
    test('should add single value', async () => {
    ...
    test('should add with selector function', async () => {
      await arrayStorage.add((value) => value.map((v) => ({ id: v.id * 10, value: v.value + 'x' })))
      expect(await arrayStorage.get()).toEqual([
        { id: 1, value: 'a' },
        { id: 2, value: 'b' },
        { id: 3, value: 'c' },
