...
function getChangeMap(oldValue?: any, newValue?: any): browser.storage.StorageChange {
      ...
      emitter.emit(
        ...
        Object.fromEntries(
          Array.from(this.store.entries())
            ...
            .map(([key]) => [key, getChangeMap(this.store.get(key), undefined)])
        )
      )
  ...
  get(keys?: unknown, callback?: unknown): void | Promise<{ [key: string]: any }> {
    ...
    const returnObject = Object.fromEntries(
      Array.from(this.store.entries()).filter(([key]) => {
        return (keys as null | undefined | string | string[])?.includes(key) ?? true
      })
    )
    if (isFunction(callback)) {
