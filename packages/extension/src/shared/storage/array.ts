import { differenceWith, isEqual, isFunction, reverse, uniqWith } from 'lodash-es'
...
import { AllowArray, AllowPromise, AreaName, BaseStorage, SelectorFn, SetterFn } from './types'

export function mergeArrayStableWith<T>(source: T[], other: T[], compareFn: (a: T, b: T) => boolean = isEqual): T[] {
...
export class ArrayStorage<T> implements IArrayStorage<T> {
  ...
  constructor(
    ...
    this.storageImplementation = new ObjectStorage<T[]>(this.defaults, optionsOrNamespace, implementations)
  ...
  public async add(value: AllowArray<T> | SetterFn<T>): Promise<void> {
    ...
    const newValue = isFunction(value) ? value(all) : Array.isArray(value) ? value : [value]
  ...
  public async remove(value: AllowArray<T> | SelectorFn<T>): Promise<T[]> {
    const all = await this.get()
    const valuesToRemove = isFunction(value) ? await this.get(value) : Array.isArray(value) ? value : [value]
