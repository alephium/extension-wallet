...
export const PasswordForm: FC<PasswordFormProps> = ({ verifyPassword, children }) => {
  const { control, formState, handleSubmit, clearErrors, setError } = useForm<FieldValues>()
    ...
    <form onSubmit={handleSubmit(handlePassword)}>
      <Controller
        ...
        render={({ field: { ref, ...field } }) => (
          <InputText autoFocus placeholder="Password" type="password" {...field} />
        )}
      />
      {errors.password?.type === 'validate' && <FormError>Password is too short</FormError>}
      {errors.password?.type === 'required' && <FormError>Password is required</FormError>}
