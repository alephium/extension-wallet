...
export const validateSeedRecoveryCompletion = (state: State): state is Required<State> =>
  Boolean(
    state.seedPhrase && state.password && validateSeedPhrase(state.seedPhrase) && validatePassword(state.password)
  )
