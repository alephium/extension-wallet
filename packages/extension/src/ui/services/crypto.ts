import { CompactEncrypt, KeyLike, exportJWK, generateSecret, importJWK, jwtDecrypt } from 'jose'
...
export const encryptForBackground = async (value: string | Uint8Array) => {
  ...
  const plaintext = isString(value) ? utf8ToArray(value) : value
  return await new CompactEncrypt(plaintext).setProtectedHeader({ alg: 'ECDH-ES', enc: 'A256GCM' }).encrypt(pubKey)
}

export const decryptFromBackground = async (encryptedValue: string, secret: Uint8Array | KeyLike) => {
...
export const generateEncryptedSecret = async () => {
  ...
  const otpBuffer = utf8ToArray(JSON.stringify(value))
  const encryptedSecret = await encryptForBackground(otpBuffer)
  return { secret, encryptedSecret }
}

function utf8ToArray(str: string): Uint8Array {
